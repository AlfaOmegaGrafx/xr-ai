...
"""Compose the 3DAIGC VLM assistant from shared SDK primitives."""
...
from .daigc_tools import mcp_endpoint, mcp_health, run_image_to_mesh
...
async def _relay_event_log(log_file: Path) -> AsyncIterator[Path]:
    ...
    lock = Lock()
    subscriber = "3daigc-vlm-compact-event-log"
...
async def run_app(
    ...
    *,
    ...
    daigc_url = mcp_endpoint(config.daigc_mcp_url)
    ...
    voice = VoiceAgent(
        ...
        probes={"vlm": vlm.health, "3daigc-mcp": lambda: mcp_health(daigc_url)},
    ...
    logger.info("3daigc-vlm-example starting")
    async with McpClient(daigc_url) as daigc_client:
        async def run_mesh(image_data_url: str) -> dict:
            return await run_image_to_mesh(
                daigc_client,
                image_data_url=image_data_url,
                model_preference=config.daigc_mesh_model,
                timeout_sec=config.daigc_job_timeout_sec,
            )

        daigc_vlm = runtime.register(
            "3daigc-vlm",
            DaigcVlmAgent(
                lambda: (
                    CurrentFrameTool(
                ...
                images=images,
                send_return_data=transport.send_return_data,
                run_mesh=run_mesh,
            ),
        )
