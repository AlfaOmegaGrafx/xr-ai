...
"""Typed configuration for the 3DAIGC VLM worker."""
...
class WorkerConfig:
    """Resolved runtime settings for one 3DAIGC VLM worker."""
...
def load_config(path: Path | None) -> WorkerConfig:
    ...
    models_name = str(data.get("models_config", "models.hosted.json"))
    idle_timeout = data.get("idle_timeout_secs")
    mesh_model = data.get("daigc_mesh_model")
    ...
    return WorkerConfig(
        ...
        frame_timeout_s=float(data.get("frame_timeout_s", 30.0)),
        ...
        daigc_mcp_url=str(data.get("daigc_mcp_url", "http://localhost:8260")),
        daigc_mesh_model=None if mesh_model is None else str(mesh_model),
